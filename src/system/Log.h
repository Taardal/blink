...
#define BL_FORMAT_LOG_MESSAGE(message) ::Blink::Log::Format(BL_FILENAME, BL_FUNCTION_NAME, BL_LINE_NUMBER, message)

#define BL_LOG_TRACE(message, ...) ::Blink::Log::Trace(BL_FORMAT_LOG_MESSAGE(message), ##__VA_ARGS__)
#define BL_LOG_DEBUG(message, ...) ::Blink::Log::Debug(BL_FORMAT_LOG_MESSAGE(message), ##__VA_ARGS__)
#define BL_LOG_INFO(message, ...) ::Blink::Log::Info(BL_FORMAT_LOG_MESSAGE(message), ##__VA_ARGS__)
#define BL_LOG_WARN(message, ...) ::Blink::Log::Warn(BL_FORMAT_LOG_MESSAGE(message), ##__VA_ARGS__)
#define BL_LOG_ERROR(message, ...) ::Blink::Log::Error(BL_FORMAT_LOG_MESSAGE(message), ##__VA_ARGS__)
#define BL_LOG_CRITICAL(message, ...) ::Blink::Log::Critical(BL_FORMAT_LOG_MESSAGE(message), ##__VA_ARGS__)
...
namespace Blink
{
    ...
    public:
        ...
        static std::string Format(const char* filename, const char* functionName, uint32_t lineNumber, std::string_view message);
